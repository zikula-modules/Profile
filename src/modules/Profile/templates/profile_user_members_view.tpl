    ...
    <tbody>
        ...
        <tr class="{cycle values='z-odd,z-even'}">
            <td>
                ...
                <a href="{modurl modname='Profile' type='user' func='onlinemembers'}">{img modname='core' src='greenled.gif' set='icons/extrasmall' __title='On-line' __alt='On-line'}</a>
                {else}
                {img modname='core' src='redled.gif' set='icons/extrasmall' __title='Off-line' __alt='Off-line'}
                {/if}
            </td>
            ...
            {/if}
            {if $msgmodule}
            <td><a href="{modurl modname=$msgmodule func='newpm' uid=$user.uid}">{img modname='core' set='icons/extrasmall' src="mail_new.gif" __alt='Send private message'}</a></td>
            {/if}
            { if isset($dudarray.url) }
            <td>
                ...
                &nbsp;
                {else}
                <a href="{$user.__ATTRIBUTES__.url|safetext}" rel="nofollow">{img modname=core set=icons/extrasmall src="agt_internet.gif" title=$user.__ATTRIBUTES__.url alt=$user.__ATTRIBUTES__.url}</a>
                {/if}
            </td>
            {/if}
            {if $adminedit}
            <td>
                <a href="{modurl modname=Users type=admin func=modify userid=$user.pn_uid}">{img modname='core' set='icons/extrasmall' src="xedit.gif" __alt='Edit'}</a>
                {if $admindelete}
                <a href="{modurl modname=Users type=admin func=deleteusers userid=$user.uid}">{img modname='core' set='icons/extrasmall' src="14_layer_deletelayer.gif" __alt='Delete'}</a>
                {/if}
            </td>
            {/if}
        </tr>
