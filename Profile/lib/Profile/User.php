...
class Profile_User extends Zikula_Controller
{
    /**
    ...
    public function view($args)
    {
        ...
        if (!empty($page)) {
            if ($this->renderer->template_exists("profile_user_view_{$page}.htm")) {
                return $this->renderer->fetch("profile_user_view_{$page}.htm", $uid);
            } else {
                return LogUtil::registerError($this->__f('Error! Could not find profile page [%s].', DataUtil::formatForDisplay($page)), 404);
            }
        }

        return $this->renderer->fetch('profile_user_view.htm', $uid);
    }
    ...
    public function modify($args)
    {
        ...
        // Return the output that has been generated by this function
        return $this->renderer->fetch('profile_user_modify.htm');
    }
    ...
    if ($this->renderer->is_cached('profile_user_members_view.htm')) {
        return $this->renderer->fetch('profile_user_members_view.htm');
    }
        */
        ...
        $this->renderer->assign('memberslistreg',    ModUtil::apiFunc('Users', 'user', 'countitems')-1); // discount annonymous
        $this->renderer->assign('memberslistonline', ModUtil::apiFunc('Profile', 'memberslist', 'getregisteredonline'));
        $this->renderer->assign('memberslistnewest', UserUtil::getVar('uname', ModUtil::apiFunc('Profile', 'memberslist', 'getlatestuser')));
        ...
        $this->renderer->assign('loggedin', UserUtil::isLoggedIn());

        // check if we should show the extra admin column
        $this->renderer->assign('adminedit', $edit);
        $this->renderer->assign('admindelete', $delete);
        ...
        $this->renderer->assign('msgmodule', ModUtil::apiFunc('Profile', 'memberslist', 'getmessagingmodule'));

        // Assign the values for the smarty plugin to produce a pager
        $this->renderer->assign('pager', array('numitems'     => $userscount,
    ...
    public function recentmembers()
    {
        ...
        // set the cache id
        $this->renderer->setCache_Id('recent' . (int)UserUtil::isLoggedIn());

        // check out if the contents are cached.
        if ($this->renderer->is_cached('profile_user_members_recent.htm')) {
        ...
        $this->renderer->assign('users', $users);
    ...
    public function onlinemembers()
    {
        ...
        // Create output object
        $this->renderer->setCache_Id = ('onlinemembers' . (int)UserUtil::isLoggedIn());

        // check out if the contents are cached.
        if ($this->renderer->is_cached('profile_user_members_online.htm')) {
            return $this->renderer->fetch('profile_user_members_online.htm');
        }
